<script setup lang="ts">
    ...
    let students = ref()
    let firstNameNewStudent:Ref<string> = ref('');
    let lastNameNewStudent:Ref<string> = ref('');

    let updatedStudentId: number;

    let editFirstName:Ref<string> = ref('')
    let editLastName:Ref<string> = ref('')
    
    let isModal:Ref<boolean> = ref(false);

    const columns: string[] = ['Index','Id','First Name','Second Name','Actions '];
    ...
    const updateStudent = async (): Promise<void> => {
        const URL: RequestInfo = `http://localhost:3000/student`;
        const requestOptions: RequestInit = {
            method: "PUT",
            headers: {
...
const deleteStudent = async (id:number): Promise<void>  => {
    ...
    const editModal = (id:number):void => {
        updatedStudentId = id;
    ...
    const redirectToStudentPage = (id:number):void => {
    window.location.pathname = "/student/" + id
};
...
<div class="min-h-full w-full items-center justify-center py-10 px-4 sm:px-6 lg:px-8 bg-stone-100 rounded-md">
 <h1 class="text-blue-600 text-2xl pb-3">Student list</h1>
    <table class="table-auto border-collapse border border-slate-400 mx-auto">
      ...
      <tbody>
        <tr v-for="(student,index) in students" @click="redirectToStudentPage(student.id)" class="cursor-pointer hover:bg-slate-200">
          ...
          <td class="border border-slate-300 py-1 px-3">
           ...
           <td class="border border-slate-300 py-1 px-3" style="width: 18%;">
            <div class="flex justify-evenly">
                <div class="cursor-pointer" @click.stop.prevent="editModal(student.id)">    
                 ...
                 <div class="cursor-pointer" @click.stop.prevent="deleteStudent(student.id)">
